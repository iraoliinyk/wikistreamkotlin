...
interface RevokedTokenCassandraRepository : CassandraRepository<RevokedToken, String>, RevokedTokenTtlRepository

interface RevokedTokenTtlRepository {
	fun saveWithTtl(token: RevokedToken, ttlSeconds: Int): RevokedToken
}
...
class RevokedTokenTtlRepositoryImpl(
	...
	private val insertWithTtlStatement: PreparedStatement = cqlSession.prepare(
		"""
	...
	override fun saveWithTtl(token: RevokedToken, ttlSeconds: Int): RevokedToken {
		val normalizedTtl = ttlSeconds.coerceAtLeast(1)
		val bound = insertWithTtlStatement.boundStatementBuilder()
